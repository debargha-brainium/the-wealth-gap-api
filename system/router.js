...
const handler = (fn) => {
    return async (req, res) => {
        try {
            ...
            if (result === null) return sendError(res, null, 'Not found', 404);
...
function parseJSONRoutes(json, router/*,errorHandler,notFoundHandler*/) {
    if (Array.isArray(json)) {
        ...
        router.use(getControllerEndpoint(json));//.controllers
    } else {
        if (json.controller) {
            ...
            stack.push(getControllerEndpoint(json));//.controllers
